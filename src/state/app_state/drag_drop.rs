...
mod tests {
    ...
    fn add_location_keeps_spawn_and_end_separate() {
        ...
        let slices = state.slices();
        let ends = slices.wave().locations(LocationType::End).unwrap();
    ...
    fn set_dragging_location_round_trips() {
        let (mut state, id) = game_with_player();
        assert!(state.slices().ui().dragging_location().is_none());
        ...
        let dragging = state.slices().ui().dragging_location().expect("ドラッグ中");
