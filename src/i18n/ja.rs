...
pub fn get_translations() -> HashMap<&'static str, String> {
    ...
    // サイドバー（単語辞書を使用）
    translations.insert(SIDEBAR_TURN_MANAGEMENT, format!("{}{}", W::TURN, W::MANAGEMENT)); // 複数単語の組み合わせ
    translations.insert(SIDEBAR_ADD_NEW_TURN, format!("新しい{}を{}", W::TURN, W::ADD));
    translations.insert(SIDEBAR_PLAYERS, format!("{}一覧", W::PLAYER));
    translations.insert(SIDEBAR_START_GAME_PROMPT, format!("{}を{}してください", W::GAME, W::START));
    ...
    translations.insert(SETUP_PLAYER_CONFIG, format!("各{}の{}:", W::PLAYER, W::CONFIG));
    translations.insert(SETUP_CONFIG_NOTE, "（実装中: デフォルト設定で開始可能）".to_string());
    ...
    translations.insert(MAIN_DRAWING_CLICK_LINE, format!("{}で{}", W::CLICK, W::LINE));
    translations.insert(MAIN_DRAWING_FREEHAND, W::FREEHAND.to_string());
    translations.insert(MAIN_DISCUSSION_INFO, format!("{}{}情報", W::DISCUSSION, W::TURN));
    ...
    translations.insert(MAIN_SET_LOCATION_BTN, format!("マップ上で{}して{}", W::CLICK, W::SET));
    translations.insert(MAIN_LOCATION_NOTE, "（実装中: マップ上で右クリックで設定予定）".to_string());
    ...
    translations.insert(MAIN_WELCOME_MESSAGE, format!("新しい{}を{}してください", W::GAME, W::START));
